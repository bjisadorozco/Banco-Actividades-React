...
const SeleccionPreguntas = () => {
  ...
  const todasLasPreguntas = [
    { id: 1, texto: "Primeros auxilios y atencion médica." },
  ...
  const validarSeleccion = () => {
    ...
    const correctCount = preguntasSeleccionadas.filter(
      (pregunta) => preguntasCorrectas.includes(pregunta)
    ).length;
          ...
          {todasLasPreguntas.map((pregunta) => (
            <div
              ...
              className={`tarjeta-pregunta ${
                seleccionadas.includes(pregunta.id) ? "seleccionada" : ""
              } ${
                resultado &&
                (preguntasCorrectas.includes(pregunta.texto) ? "correcto" : "incorrecto")
              }`}
          ...
          <p className="text-md mt-0 font-bold text-center">
            {resultado === "correcto" ? (
              ...
              <span>
                ...
                <span style={{ color: '#8F8F8F' }}>
                  Tienes {correctCount} de {preguntasCorrectas.length} respuestas correctas. Intenta de nuevo. ({Math.round((correctCount / preguntasCorrectas.length) * 100)}%)
                </span>
              </span>
            )}
          </p>
